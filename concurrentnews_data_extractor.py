...
def process_single_file(file_path):
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            print(f"Extracting elements from file '{os.path.basename(file_path)}':")
            ...
            if not isinstance(security_summary, dict):
                print(f"Warning: 'securitySummary' is not a dictionary in file '{os.path.basename(file_path)}'. Skipping this file.")
                return None
            ...
            if isinstance(news, dict):
                ...
                print(f"Warning: Unexpected format for 'news' in file '{os.path.basename(file_path)}': {type(news)}")
            ...
            if isinstance(events, list):
                ...
                print(f"Warning: 'events' is neither a list nor a dictionary in file '{os.path.basename(file_path)}'. Skipping this section.")
        ...
        print(f"Error: Failed to decode JSON in file '{os.path.basename(file_path)}': {e}")
    except FileNotFoundError as e:
        print(f"Error: File '{os.path.basename(file_path)}' not found: {e}")
...
def process_json_files_concurrently(folder_path, test_mode=False, test_file=None):
    json_files = [f for f in os.listdir(folder_path) if f.endswith('.json')]
    if test_mode:
        json_files = [test_file or random.choice(json_files)]
        print(f"Testing mode: Processing specified or random file '{json_files[0]}'")

    results_folder = os.path.join(folder_path, "processed_results")
    os.makedirs(results_folder, exist_ok=True)

    with ThreadPoolExecutor(max_workers=4) as executor:
        futures = {executor.submit(process_single_file, os.path.join(folder_path, filename)): filename for filename in json_files}
...
if __name__ == "__main__":
    folder_path = '/home/a/Fin Project/Financial Web Scraping/data'  # Replace with your folder path
    process_json_files_concurrently(folder_path, test_mode=False)
